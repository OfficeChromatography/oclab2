...
unsigned int ink_fire_delay = 800;
uint8_t pulse_pin;
#endif
#ifdef FWRETRACT
...
void process_commands()
{
  ...
  else if(code_seen('M'))
  {
    ...
    case 150: // M150 Neopixel LEDs
      ...
      if (code_seen('R')){
	red =  check_if_valid_color_value(code_value());
      }
      if (code_seen('U')){
	green= check_if_valid_color_value(code_value());
      }
      if (code_seen('B')){
	blue = check_if_valid_color_value(code_value());
      }
      if (code_seen('W')){
	white = check_if_valid_color_value(code_value());
      }
      set_neopixel_color(red,green,blue,white);
      break;
    ...
    break;
            ...
            case 700:
              ...
              {
                ink_pulse_delay = (int) code_value();
              }
	      if (code_seen('D')) // D parameter for nozzle fire Delay
		{
		  ink_fire_delay = (int) code_value();
		}
              if (code_seen('P'))
              {
                switch((int)code_value())
		      ...
		      delayMicroseconds(ink_fire_delay);			
